...
# #         self.client = OpenAI(api_key='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')
...
#                 model="gpt-3.5-turbo",  # Use 'gpt-3.5-turbo' or the appropriate model available
#                 messages=[
...
# Working code with OpenAI (commented out) ------------------------------------------------------------------------------------------------------------------------------
...
# --------------------------------------------------------------------------------------------------------------------------------------
# Working code with Gemini (commented out) ------------------------------------------------------------------------------------------------------------------------------
...
#         self.client = genai.GenerativeModel('gemini-1.5-flash')  # Use Gemini 1.0 Pro
#         self.use_mock = os.getenv('USE_MOCK', 'False').lower() == 'true'
...
#         retry=retry_if_exception_type((json.JSONDecodeError, GoogleAPIError, ResourceExhausted))
#     )
#     def _call_gemini(self, prompt: str, system_content: str, temperature: float = 0.3) -> str:
#         """Internal API call to Gemini with retry"""
#         try:
#             # Gemini doesn't use system messages directly; combine system_content with prompt
...
#             logging.error(f"Gemini API rate limit exceeded: {str(e)}")
#             raise
...
#     def analyze_cv_assignment_match(self, cv_text: str, assignment_text: str) -> Dict[str, Any]:
#         """Analyze CV against assignment requirements using Gemini"""
#         if self.use_mock:
#             logging.debug("Using mock analysis response")
#             return self._mock_analysis()
...
#                 "requirements": [],
#                 "wishes": [],
#                 "error": f"Rate limit exceeded. Please try again later: {str(e)}"
#             }
#         except GoogleAPIError as e:
#             logging.error(f"Gemini API error: {str(e)}")
#             return {
...
#                 motivations[req['id']] = content
#             except ResourceExhausted as e:
#                 motivations[req['id']] = f"Rate limit exceeded: {str(e)}"
#             except GoogleAPIError as e:
#                 motivations[req['id']] = f"Gemini API error: {str(e)}"
#             except Exception as e:
#                 motivations[req['id']] = f"Error generating motivation: {str(e)}"
...
# Working code with Ollama (uncommented) ------------------------------------------------------------------------------------------------------------------------------
...
#     def analyze_cv_assignment_match(self, cv_text: str, assignment_text: str) -> Dict[str, Any]:
#         """Analyze CV against assignment requirements using deepseek-r1:1.5b"""
#         if self.use_mock:
#             logging.debug("Using mock analysis response")
#             return self._mock_analysis()
...
class CVAnalyzer:
    ...
    def generate_motivations(self, cv_text: str, requirements: List[Dict], consultant_name: str) -> Dict[str, str]:
        ...
        for req in requirements:
            ...
            - Returns plain text with no additional formatting or code fences
            ...
            try:
                content = self._call_ollama(
                    prompt,
                    "You are an expert HR consultant. Return plain text with no additional formatting or code fences.",
                    temperature=0.3
                )
    ...
    def generate_introduction_email(self, consultant_info: Dict, assignment_info: Dict, analysis_result: Dict) -> str:
        ...
        try:
            content = self._call_ollama(
                prompt,
                "You are an expert staffing agency representative. Return plain text with no additional formatting or code fences.",
                temperature=0.3
            )
            return content.strip()
        except ollama.ResponseError as e:
