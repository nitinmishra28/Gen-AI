...
from src.analyzer.prompts import (
    get_analyze_cv_prompt, 
    get_cover_letter_prompt, 
    get_customize_content_prompt, 
    get_motivation_prompt,
    get_introduction_email_prompt
)
...
class CVAnalyzer:
    ...
    async def generate_motivations(self, cv_text: str, requirements: List[Dict], 
                                 consultant_name: str, model_name: Optional[str] = None) -> Dict[str, str]:
        """Generates personalized motivations for each requirement using prompts.py. Now async."""
        if self.use_mock: 
            return {req['id']: "Mock motivation for testing." for req in requirements}
        ...
        async def process_requirement(req):
            try:
                ...
                prompts = get_motivation_prompt(cv_text, req['title'], req, model_name)
                content = await self._call_llm_async(
                    prompt=prompts['user'], 
                    system_content=prompts['system'], 
                    model_name=model_name
    ...
    async def generate_cover_letter(self, cv_text: str, assignment_info: Dict, consultant_name: str, 
                                  analysis_result: Dict, model_name: Optional[str] = None) -> str:
        """Generates a professional cover letter using prompts.py. Now async."""
        if self.use_mock: 
            return "Mock cover letter for testing."
        if not model_name: 
            model_name = config.DEFAULT_OPENAI_MODEL
        ...
        prompts = get_cover_letter_prompt(
            cv_text=cv_text,
            job_desc=assignment_description,
            model_name=model_name,
    ...
    async def generate_introduction_email(self, consultant_info: Dict, assignment_info: Dict, 
                                        analysis_result: Dict, model_name: Optional[str] = None) -> str:
        """Generates a professional introduction email using prompts.py. Now async."""
        if self.use_mock: 
            return "Mock email for testing."
        if not model_name: 
            model_name = config.DEFAULT_OPENAI_MODEL
        ...
        # Use the centralized prompt function
        prompts = get_introduction_email_prompt(
